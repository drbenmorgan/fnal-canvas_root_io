...
include(${CMAKE_CURRENT_LIST_DIR}/CheckClassVersion.cmake)
...
function(_generate_dictionary dictname)
  ...
  endforeach()
  # And for all interface dirs of the dict?
  set(_prop_helper "$<TARGET_PROPERTY:${dictname}_dict,INCLUDE_DIRECTORIES>")
  set(GENREFLEX_INCLUDES $<$<BOOL:${_prop_helper}>:-I$<JOIN:${_prop_helper},;-I>>)

  # Add current compile definitions
  get_directory_property(compile_defs COMPILE_DEFINITIONS)
  foreach(def ${compile_defs})
    # Does NOT like genexps - if these expand to nothing get an empty -D arg!!
  ...
  if(HAVE_ROOT6)
    list(APPEND GENREFLEX_FLAGS -l $<TARGET_PROPERTY:${dictname}_dict,LIBRARY_OUTPUT_DIRECTORY>/${CMAKE_SHARED_LIBRARY_PREFIX}${dictname}_dict${CMAKE_SHARED_LIBRARY_SUFFIX})
  endif()
  ...
  if(BD_WANT_CAP_FILE)
    ...
    list(APPEND GENREFLEX_FLAGS --capabilities=${SOURCE_OUTPUT})
  endif()

  if(BD_WANT_PCM)
    set(PCM_OUTPUT $<TARGET_PROPERTY:${dictname}_dict,LIBRARY_OUTPUT_DIRECTORY>/${CMAKE_SHARED_LIBRARY_PREFIX}${dictname}_dict_rdict.pcm)
...
function (build_dictionary)
  ...
  if(BD_DICTIONARY_LIBRARIES)
    ...
    # Suspect this is not needed when full paths and imported targets used consistently
  ...
  list(APPEND dictionary_liblist ROOT::Core ) #R5 only? #ROOT::Reflex)
  ...
  # Root currently does not set interface properties, so add explicitly
  target_include_directories(${dictname}_dict PRIVATE ${ROOT_INCLUDE_DIRS})

  if(BD_WANT_ROOTMAP)
    set(ROOTMAP_OUTPUT "$<TARGET_PROPERTY:${dictname}_dict,LIBRARY_OUTPUT_DIRECTORY>/${CMAKE_SHARED_LIBRARY_PREFIX}${dictname}_dict.rootmap")
    _generate_dictionary(${dictname} ROOTMAP_OUTPUT ${ROOTMAP_OUTPUT} PCM_OUTPUT_VAR PCM_OUTPUT)

    # Need keep track of pcm/rootmap, so store as properties of the lib
    set_target_properties(${dictname}_dict
      PROPERTIES
        ROOTMAP_FILE ${ROOTMAP_OUTPUT}
        PCM_FILE ${PCM_OUTPUT}
        )
  else()
  ...
  if(BD_COMPILE_FLAGS)
    set_target_properties(${dictname}_dict PROPERTIES COMPILE_FLAGS ${BD_COMPILE_FLAGS})
    if(BD_WANT_CAP_FILE)
      set_target_properties(${dictname}_map PROPERTIES COMPILE_FLAGS ${BD_COMPILE_FLAGS})
    endif()
  endif()
